/// "60s" | "2m" | "2h" | "7d" | bare seconds -> seconds.
pub(crate) fn parse_duration_secs(s: &str) -> Result<u64, String> {
    ...
    digits.trim().parse::<u64>().map(|n| n * mult).map_err(|_| {
        format!("invalid duration '{s}' (expected like \"60s\", \"2m\", \"2h\" or \"7d\")")
    })
}
...
mod tests {
    ...
    #[test]
    fn parse_duration_secs_supports_days() {
        assert_eq!(parse_duration_secs("7d").unwrap(), 7 * 24 * 3600);
