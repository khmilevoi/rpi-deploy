...
pub async fn uninstall(sys: &dyn Sys, opts: &UninstallOpts) -> UninstallReport {
    ...
    if sys.run("systemctl", &["disable", "--now", "pi-agent"]).await.is_err() {
        rep.warnings.push("systemctl disable --now pi-agent failed (may already be stopped)".into());
    }
    ...
    if user_exists(sys, "pi-agent").await {
        if sys.run("userdel", &["pi-agent"]).await.is_ok() {
            rep.removed.push("user pi-agent".into());
        } else {
            rep.warnings.push("userdel pi-agent failed (user may be in use)".into());
        }
    }
