...
///
/// Also pins `restart: unless-stopped` on the public service. pi owns this
...
mod tests {
    ...
    #[test]
    fn yaml_pins_unless_stopped_so_container_survives_reboot() {
        // Without a restart policy the deployed container inherits Docker's
        // default `no`, so it stays down after a host reboot (the public
        // service never comes back until someone runs `docker start`). pi
        // owns this service's lifecycle, so the generated override must pin a
        // durable restart policy.
        let yaml = override_yaml("web", "127.0.0.1", 8000, 3000);
        assert!(
            yaml.contains("restart: unless-stopped"),
            "override must pin restart: unless-stopped; got:\n{yaml}"
        );
    }
