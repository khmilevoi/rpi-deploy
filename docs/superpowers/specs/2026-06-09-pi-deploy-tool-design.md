...
- **Restart-политика — через тот же override.** Тот же сгенерированный override
  пинит `restart: unless-stopped` на публичном сервисе: `pi` владеет его
  жизненным циклом, поэтому контейнер обязан пережить ребут хоста без ручного
  вмешательства. Без явной политики контейнер наследует дефолт Docker `no` и
  после ребута остаётся лежать (сайт отдаёт 530, пока кто-то не сделает
  `docker start`). Именно `unless-stopped`, а не `always`, — чтобы осознанный
  `pi stop` уважался и после ребута.
