...
2. **Spawn three read-only auditor subagents in parallel** (one message; Explore-type agents fit — they must not edit anything). Narrow lenses are the point: one broad "check the landing" pass skims and misses, three auditors each reading their sources end to end don't. Each prompt must be self-contained: the site repo path, the pi repo path, the absolute path to this skill's `references/landing-audit.md` with which section to follow, and the report format defined there.
   ...
   - **Auditor 2 — CLI output fidelity**: the hero terminal and quick-start output snippets are **transcriptions of real `rpi` output, not mocks** — every marker (`▸`, not `●`), glyph, colour, stage name and wording must be reproducible from the rendering code (`crates/bin/src/output/{theme,banner,pipeline}.rs`, `crates/application/src/deploy.rs`) and **re-derived against this release's build every time** (they depict this site's own `rpi-deploy-site` deploy, and drift as the CLI evolves). Flag any invented or stale line.
...
3. **Apply the confirmed fixes yourself** in the site repo (auditors only report). When you rewrite any terminal block, transcribe it from the rendering code — the canonical deploy transcript and colour map in the Auditor 2 brief of `references/landing-audit.md` — never from memory or invention.
...
5. **Deploy and verify**: commit (include the regenerated `og.png`), push, then `rpi deploy` from the site repo root; check the live page reflects the fixes and re-fetch the OG (`og:image`) to confirm it is the new hero. The npm version *badge* is the only element that updates itself — every other number and claim on the page is hand-written.
