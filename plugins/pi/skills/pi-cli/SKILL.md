---
name: pi-cli
description: Use when operating, installing, testing, or troubleshooting the pi deploy CLI, including pi deploy, pi ls, pi env send, pi env ls, pi gc, pi agent run, SSH profiles, PI_SERVER, PI_AGENT_URL, local dev agents, and CLI-to-agent connection failures.
---
...
Use this skill for commands and workflows around the `pi` binary. Treat the repository README and CLI source as the source of truth when behavior has drifted.
...
| Task | Command |
| --- | --- |
| Deploy current project | `pi deploy` |
| Deploy a ref | `pi deploy --ref <branch-tag-or-sha>` |
| Cancel active deploys for current `pi.toml` project | `pi deploy --cancel` |
| List projects | `pi ls` or `pi ps` |
| Send env bundle | `pi env send` |
| Send env bundle and restart running stack | `pi env send --apply` |
| List stored env keys | `pi env ls` |
| Prune agent Docker images/build cache | `pi gc` |
| Run foreground agent | `pi agent run --config <agent.toml>` |
...
4. If `[env] file = ".env"` is used and secrets are required, run `pi env send` before the first deploy.
5. Prefer Compose `expose` for the managed service; avoid fixed host `ports` that conflict with pi's allocator.
...
- Compose does not see secrets: run `pi env send`, or `pi env send --apply` for an already running stack.
...
- Host port conflict: remove fixed host `ports:` from Compose and let pi write the override.
